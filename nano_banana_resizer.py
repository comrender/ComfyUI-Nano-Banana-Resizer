...
class NanoBananaSizeCalculator:
    ...
    # Preserve original bucket-selection behavior from legacy version.
    LEGACY_BUCKET_ARS = {
        "1:1": 1.0,
        "3:2": 1.5,
        "2:3": 0.666667,
    ...
    def _detect_aspect_ratio(self, w: int, h: int) -> str:
        ...
        for ar_str, ar_val in self.LEGACY_BUCKET_ARS.items():
    ...
    def _best_bucket_for_outlier(
        ...
        target_ar = self.LEGACY_BUCKET_ARS.get(target_ar_str, w_in / h_in)
    ...
    def _closest_bucket(
        ...
        # Override 3: Gemini 4K portrait edge cases observed around 4:5-ish inputs.
        if (
            (3712, 4608) in buckets
            and 3330 <= w_in <= 3660
            and 4260 <= h_in <= 4660
            and 0.775 <= (w_in / h_in) <= 0.790
        ):
            return (3712, 4608)
    ...
    def calculate_size(self, image, preset: str, method: str):
        ...
        # Keep original sizing behavior from the old version.
